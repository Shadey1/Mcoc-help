...
type CellResult =
  | { kind: 'blank' }
  | { kind: 'hint' } // still carries the `?` prefix — user hasn't verified
...
function parseCell(raw: string): CellResult {
  ...
  // `?<mark>` cells are pre-fills generated by the template writer to show
  // the user what other sources currently claim — they only become chart
  // votes when the user removes the leading `?`. See generate-chart-template.ts.
  if (value.startsWith('?')) return { kind: 'hint' };
...
function main() {
  ...
  for (const row of rows.slice(1)) {
    ...
    // unmatched name doesn't produce a spurious warning. Rows carrying
    // only `?` hints (not yet verified) count as "no content" so the
    // ingestion feedback isn't dominated by pre-fills the user hasn't
    // touched.
    const cells: Array<{ effect: EffectName; result: CellResult }> = [];
    let hasBand = false;
    for (const effect of EFFECTS) {
      const result = parseCell(row[effectCol.get(effect)!] ?? '');
      if (result.kind === 'band') hasBand = true;
      cells.push({ effect, result });
    }
    if (!hasBand) {
      // Row has nothing verified — count hints in it so the summary
      // shows the reviewer what's still on the table.
      for (const { result } of cells) if (result.kind === 'hint') unverifiedHints++;
      blankRows++;
      continue;
    }
  ...
  console.log(
    `Ingested ${ingestedCells} cells across ${Object.keys(champions).length} champions. ` +
      ...
      `Unverified hints still in template: ${unverifiedHints}.`,
  );
