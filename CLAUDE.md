...
- **Imports never make the data worse.** The abilities importer merges: a card MCOCHUB stops showing is kept and tagged `retainedFrom` (the page says so), a page that parses to nothing keeps its previous kit, and if the numbers change on most lines at once the import is refused (exit 3) until a person passes `--accept-numbers`. Every previous version is in git; `git show <commit>:data/champions/abilities.json` recovers any of them.
